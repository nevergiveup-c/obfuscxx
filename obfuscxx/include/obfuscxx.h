...
template<size_t N>
FORCEINLINE consteval uint64_t hash_compile_time(char const (&data)[N]) {
    uint64_t hash = 0;
...
public:
    ...
    consteval obfuscxx(const std::initializer_list<Type> &list) {
    ...
    public:
        ...
        const CharType &operator[](size_t i) const { return data[i]; }
    ...
    struct array_copy {
        ...
        constexpr size_t size() const { return N; }
        constexpr size_t size_bytes() const { return N * sizeof(ArrayType); }
